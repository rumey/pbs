...
class Migration(DataMigration):
    ...
    fmsb_group, c = Group.objects.get_or_create(name='Fire Management Services Branch')
    drfms_group, c = Group.objects.get_or_create(name='Director Fire and Regional Services')
    ...
    def forwards(self, orm):
        ...
        filename = os.path.join(settings.BASE_DIR, 'pbs/review/migrations', 'sdo_mark_reviewed.csv')
        reviews = []
        ...
        FMT = '%Y-%m-%dT%H:%M:%S'
        for e in reviews:
            prescription = orm['prescription.Prescription'].objects.get(burn_id=e[0])
            user = get_user_model().objects.get(username=e[1].lower())
            review_type = e[2]
            review_date = datetime.datetime.strptime(e[3], FMT)

            # check user has permissions
            if ((review_type == 'FMSB' and self.fmsb_group in user.groups.all()) or
                (review_type == 'DRFMS' and self.drfms_group in user.groups.all())):

                burnstate, created = orm.BurnState.objects.get_or_create(prescription_id=prescription.id, user_id=user.id, review_type=review_type)
                burnstate.review_date = review_date #datetime.datetime.now()
                burnstate.save()
                if created:
                    print('CREATED: {}'.format(burnstate))
                else:
                    print('UPDATED: {}'.format(burnstate))
            else:
                # check if user is in wrong group
                if ((review_type == 'FMSB' and self.drfms_group in user.groups.all()) or
                    (review_type == 'DRFMS' and self.fmsb_group in user.groups.all())):
                    print('USER ERROR: Incorrect Group {} - user {} is in group {} '.format(review_type, user.username, user.groups.all()))
                else:
                    print('USER ERROR: user {} is NOT in Group {}'.format(user.username, review_type))
