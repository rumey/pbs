...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for prescription in orm['prescription.Prescription'].objects.all():
            count = Document.objects.filter(
                tag__name="Fuel Assessment Summary_FIRE 872",
                prescription=prescription).count()
            print "%s found %s fuel assessment(s)" % (
                prescription, count)
            if count or prescription.status == Prescription.STATUS_CLOSED:
                day_state = prescription.day_state
                day_state.fuel_assessment = True
                day_state.save()
    ...
    models = {
        ...
        u'report.areaachievement': {
            ...
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'ignition': ('django.db.models.fields.DateField', [], {'default': 'datetime.datetime(2014, 1, 28, 0, 0)'}),
