...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for prescription in orm['prescription.Prescription'].objects.all():
            overview = orm.OperationalOverview.objects.get_or_create(prescription=prescription, creator=admin, modifier=admin)

            #if prescription.endorsement_status != 1:
            #    # This prescription has already been submitted for endorsement
            #    # which means it must be "complete".
            #    day_state = prescription.day_state
            #    day_state.operational_overview = True
            #    day_state.save()
