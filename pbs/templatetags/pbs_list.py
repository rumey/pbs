...
def result_headers(cl):
    ...
    for i, field_name in enumerate(cl.list_display):
        ...
        if hasattr(cl.model_admin.model, '_required_fields') and \
           field_name in getattr(cl.model_admin.model, '_required_fields') and \
           cl.list_editable:
...
def has_editable_fields(cl, request):
    """
    Returns true if any of the forms in the fieldset have more than just
    one field.
    """
    return (any(len(form.fields) > 1 for form in cl.formset.forms)
            or cl.model_admin.get_list_editable(request) and
            cl.model_admin.list_empty_form)
...
def ifusergroup(parser, token):
    ...
    Via: https://djangosnippets.org/snippets/2736/
    """
    try:
