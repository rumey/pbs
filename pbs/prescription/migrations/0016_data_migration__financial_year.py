...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for p in Prescription.objects.all():
            s=Season.objects.filter(name=p.planned_season)[0]
            if s:
                p.planned_season = Season.SEASON_ANNUAL
                if s.start.month <= 6:
                    # reset the planned_year to the 1st part of the fncl_year
                    p.planned_year = p.planned_year-1
                p.financial_year = str(p.planned_year) + '/' + str(p.planned_year+1)
                p.save()
            else:
                raise Exception('ERROR: No Season Found')

        for p in Prescription.objects.all():
            for season in Season.SEASON_CHOICES:
                if p.last_season:
                    p.last_season = self.seasons[int(p.last_season)]
                    p.save()
    ...
    def backwards(self, orm):
        ...
        # revert field changed from int to char
        d = eval(open('scripts/last_season.txt', 'r').read())

        for pid in d.keys():
            p = Prescription.objects.get(id=pid)
            p.last_season = pid
            p.save()

        # revert financial_year
        # pass
