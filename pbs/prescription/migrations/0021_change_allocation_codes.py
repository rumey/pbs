...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for fa in orm.FundingAllocation.objects.all():
            if ( fa.allocation in [04, 24, 32, 41] and
                 fa.prescription.ignition_status != IGNITION_COMPLETE ):
                if fa.allocation == 04:
                    fa.allocation = 7204
                else:
                    fa.allocation = 72
    ...
    def backwards(self, orm):
        ...
        for fa in orm.FundingAllocation.objects.all():
            if ( fa.allocation in [72, 7204] and
                 fa.prescription.ignition_status != IGNITION_COMPLETE ):
                if fa.allocation == 7204:
                    fa.allocation = 04
                else:
