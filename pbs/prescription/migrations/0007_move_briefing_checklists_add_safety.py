...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        summary = orm.SMEAC.objects.get_or_create(category='Briefing Summary')[0]
        safety = orm.SMEAC.objects.get_or_create(category='Safety')[0]
        admin = orm['auth.User'].objects.get(pk=1)
        for prescription in orm.Prescription.objects.all():
            checklists = orm.BriefingChecklist.objects.filter(prescription=prescription, smeac=safety)
            checklists.update(smeac=summary)
            orm.BriefingChecklist.objects.create(prescription=prescription,
                smeac=safety, title='Safety Issues', creator=admin, modifier=admin)

    def backwards(self, orm):
        "Write your backwards methods here."
        summary = orm.SMEAC.objects.get(pk=7)
        safety = orm.SMEAC.objects.get(pk=6)
        for prescription in orm.Prescription.objects.all():
            orm.BriefingChecklist.objects.get(prescription=prescription,
                smeac=safety, title='Safety Issues').delete()
            checklists = orm.BriefingChecklist.objects.filter(prescription=prescription, smeac=summary)
            checklists.update(smeac=safety)
