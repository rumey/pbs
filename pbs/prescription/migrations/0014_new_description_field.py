...
def location_text(prescription):
    """From the Prescription property"""
    if prescription.location:
        value = prescription.location.split('|')
        if len(value) > 1:
...
def generate_description(prescription):
    """
    Taken from the computed property on the Prescription model.
    """
    if prescription.tenures.count() > 0:
        located = ", ".join(tenure.name for tenure in prescription.tenures.all())
    else:
        located = "TBD"

    if prescription.last_year_unknown:
        last_burnt = "is unknown"
    else:
        last_burnt = "was %s" % prescription.last_year

    blocks = " in %s" % prescription.forest_blocks if prescription.forest_blocks else ""
    ...
    return (
        ...
        'located': located,
        'location_text': location_text(prescription) or "TBD",
        'forest_blocks': blocks,
        'veg_types': ", ".join(unicode(v) for v
                                in prescription.fuel_types.all()) or "TBD",
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        qs = list(orm.Prescription.objects.all())
        for p in qs:
    ...
    def backwards(self, orm):
        ...
        for p in orm.Prescription.objects.all():
            p.description = ""
            p.save()
    ...
    models = {
        ...
        u'prescription.prescription': {
            ...
            'financial_year': ('django.db.models.fields.CharField', [], {'default': '1', 'max_length': '10'}),
